    ...
    public class SliderController : Controller
    {
        ...
        public ActionResult Edit([Bind(Include = "SliderId,Baslik,Aciklama,ResimURL")] Slider slider, HttpPostedFileBase ResimURL,int id)
        {
            if (ModelState.IsValid)
            {
                ...
                if (ResimURL != null)
                {
                    if (System.IO.File.Exists(Server.MapPath(s.ResimURL))) //daha önce kayıtlı dosya var mı kontrol ediyoruz.
                    {
                        System.IO.File.Delete(Server.MapPath(s.ResimURL)); //varsa sil o dosyayı.
                    }
                    WebImage img = new WebImage(ResimURL.InputStream);
                    FileInfo imginfo = new FileInfo(ResimURL.FileName);

                    // logo isminin atanması boyutunun belirlenmesi.
                    string slinderimgname = Guid.NewGuid().ToString() + imginfo.Extension;
                    img.Resize(1024, 360);
                    img.Save("~/Uploads/slider/" + slinderimgname);

                    s.ResimURL = "/Uploads/slider/" + slinderimgname;
                }
